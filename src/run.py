...
def run(argv=sys.argv):
    ...
    print(f"Used dataset and checkpoints will be saved at: \n{BASE_DIR} \n") # Get package install path

    if not os.path.exists(BASE_DIR):
        BASE_DIR.mkdir(parents=True)
        shutil.copytree(PKG_DIR / "data", BASE_DIR / "data")
        shutil.copytree(PKG_DIR / "checkpoints", BASE_DIR / "checkpoints")
        shutil.copytree(PKG_DIR / "reports", BASE_DIR / "reports")

    dataset_download.download_dataset(str(BASE_DIR))
    ...
    def update_global_model(ckpt_name):
        nonlocal current_model, current_ckpt
        if not ckpt_name:
            current_model = None
            current_ckpt = None
    ...
    def run_training(batch_size, max_epochs, lr):
        ckpt_path, metrics_path = train_model(str(BASE_DIR), batch_size=batch_size, max_epochs=max_epochs, lr=lr)
    ...
    def load_metrics_global(ckpt_name):
        ...
        metrics_path = os.path.join(BASE_DIR, "reports", "data", f"{os.path.splitext(ckpt_name)[0]}_metrics.pkl")
